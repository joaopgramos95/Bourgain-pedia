...
\theoremstyle{plain}
\newtheorem*{propAK}{Proposition (ours: the Alexandrov--Kargaev inequality)}
\theoremstyle{definition}
\theoremstyle{remark}
...
\end{enumerate}
\smallskip
\noindent And one that is not typographical:
\begin{enumerate}[label=(\roman*),leftmargin=2.2em,start=6]
\item p.~4: Lemma 2 is stated with no factor $I(a_Q)$ multiplying
  $dF_\varepsilon/dn$. As printed the statement is not homogeneous in $I$ and is
  \emph{false} for large $|I(a_Q)|$; its proof establishes only the normalised case
  $I(a_Q)=1$; and Lemma 4 applies it to a term that does carry the factor. Restoring
  the factor makes statement, proof and application agree. \Ledger{C9}, \S3.
\end{enumerate}

\smallskip
None of the six affects the theorem. Two further points needed real repair rather
...
\medskip
\noindent\textbf{Nothing is left unclosed.} The paper's one load-bearing import --- the
Alexandrov--Kargaev inequality, cited to a private communication we could not obtain
--- is \emph{proved} in \S2.2 rather than assumed. The proof is ours and gives only
the special case Lemma 1 needs; its centrepiece is the exact evaluation
\[\int_0^\infty\log\bigl|1-u^{-d}\bigr|u^{d-2}\,du=-\frac{\pi}{d-1}\cot\frac{\pi}{d},\]
which is negative precisely for $d\ge3$ and vanishes at $d=2$. So the hypothesis
$d\ge3$ turns out to be visible twice: in the convergence of $\sum\varepsilon_n^{(d-1)/2}$
(\S7), and in the sign of the imported inequality itself.

The two `caution' boxes that remain, in \S2.6 and \S3.3, flag steps of the original
that are repaired on the spot; neither is an open gap.

% =====================================================================
\section*{\S0.5. Why the question matters}
% =====================================================================

\begin{ournotation}
\textbf{This section is context, and it is not the authors'.} The paper offers no
motivation --- it says only that ``the reader may wish to consult [2] for a
motivation'' --- and the framing below is later than 1989. It is drawn from
R.~C.~D.~de Andrade's master's thesis (\S9(6)), chapter 1, which is careful to record
that ``the work of Bourgain and Wolff was not originally motivated by the Calder\'on
problem, but rather by fundamental questions in harmonic analysis concerning boundary
behavior and uniqueness for harmonic functions.'' We keep it here rather than in \S9
because a reader meeting the theorem cold deserves to know what is at stake; nothing
in the proof depends on any of it.
\end{ournotation}

\noindent\textbf{The inverse problem.} In electrical impedance tomography one applies
voltages on the boundary of a body, measures the resulting currents, and tries to
recover the conductivity inside. The data is the Dirichlet-to-Neumann map
$\Lambda_\gamma: f\mapsto\gamma\,\partial_\nu u|_{\partial\Omega}$, and Calder\'on's
problem asks whether $\Lambda_\gamma$ determines $\gamma$. It does, in considerable
generality. But the reconstruction is severely ill-posed: the modulus of continuity in
the known stability estimates is only logarithmic.

\medskip
\noindent\textbf{What this theorem says about that.} Take the simplest case,
$\gamma\equiv1$, so the boundary flux is just the normal derivative and the interior
equation is $\Delta u=0$. The theorem produces a harmonic $f\not\equiv0$ whose trace
\emph{and} normal derivative vanish together on a boundary set of positive measure. In
tomographic terms: on a substantial part of the boundary there is neither applied
voltage nor measured current, and yet the potential inside is not zero. That part of
the interior is invisible to the measurement.

This does not contradict unique continuation in the \emph{interior}, which holds. What
it shows is that boundary data can degenerate on a large set --- a concrete mechanism
for the instability, rather than merely a bound that happens to be weak.

\medskip
\noindent\textbf{And it is dimension-dependent.} In the plane the same statement is
false: if the Dirichlet and Neumann data of a harmonic function both vanish on a set
of positive boundary measure, the function vanishes identically (\S9(7)). So the
phenomenon appears exactly when $d\ge3$ --- which is the hypothesis of the theorem,
and which \S2.2 and \S7 both locate inside the proof.
...
\[\nd h(x)=c_d'\int_{\Rdm}|x-y|^{-d}h(y)\,dy\]
for a constant $c_d'$ depending only on $d$. This is \eqref{eq:dtn} written as a
singular integral: $|\nabla'|$ has convolution kernel a constant times $|z|^{-d}$
on $\Rdm$, understood in the principal-value sense, and off $\supp h$ no principal
value is needed because the kernel is smooth there. Only $|c_d'|\lesssim1$ is used
below; we do not need its value.
\end{bg}
...
make the integral \eqref{eq:AK} of \S2.2 below negative. With the outward convention every
regime contributes a positive amount --- up to a negative term carried by a set of
measure $O(\varepsilon^{d-1})$ around the single zero of $1+\nd G_\varepsilon$, which
vanishes with $\varepsilon$ --- so \eqref{eq:AK} is false. \S9(5) confirms this at
leading order in $p$. So the inward normal is the paper's convention, and we adopt it.
\end{filled}
...
\begin{filled}
\textbf{We prove \eqref{eq:AK}.} \Ledger{B4} The original imports this from [1], a
private communication we could not obtain (see \S9(1)). Rather than leave the paper's
one load-bearing input unverified, we prove the statement Lemma 1 actually needs.
The proof is ours; Aleksandrov and Kargaev presumably prove more, and in a different
form.

\begin{propAK}
Let $d\ge3$. There is $p_0=p_0(d)\in\bigl(0,\tfrac{d-1}{d}\bigr)$ such that for every
$p\in(0,p_0)$ there are $M=M(p,d)>0$ and $\varepsilon_0=\varepsilon_0(p,d)>0$ with
\[\int_{\Rdm}\Bigl(\bigl|1+\nd G_\varepsilon\bigr|^p-1\Bigr)dx<-2M<0
  \qquad\text{for all }0<\varepsilon<\varepsilon_0 .\]
For $d=2$ the conclusion fails: the integral is positive for all small $p$ and
$\varepsilon$.
\end{propAK}

Write $\Phi_\varepsilon(x)=1+\nd G_\varepsilon(x)$. By \eqref{eq:dnG},
\begin{equation}\label{eq:Phieps}
 \Phi_\varepsilon(x)=1-\bigl(|x|^2+\varepsilon^2\bigr)^{-\frac{d+2}2}
   \bigl(|x|^2-(d-1)\varepsilon^2\bigr),
 \qquad
 \Phi_0(x):=1-|x|^{-d},
\end{equation}
and $\Phi_\varepsilon\to\Phi_0$ pointwise on $\Rdm\setminus\{0\}$. Put
\[I(\varepsilon,p)=\int_{\Rdm}\bigl(|\Phi_\varepsilon|^p-1\bigr)dx,
  \qquad
  J(p)=\int_{\Rdm}\bigl(|\Phi_0|^p-1\bigr)dx
      =\omega_{d-2}\!\int_0^\infty\!\bigl(|1-u^{-d}|^p-1\bigr)u^{d-2}du,\]
$\omega_{d-2}=|S^{d-2}|$. Both converge for $0<p<\frac{d-1}{d}$: near $u=0$ the
integrand is $O(u^{d-2-dp})$, near $u=1$ it has an integrable logarithmic
singularity, and near $u=\infty$ it is $O(u^{-2})$.

\medskip
\noindent\textbf{Step 1: an exact evaluation.} Let
\[L(d)=\int_0^\infty\log\bigl|1-u^{-d}\bigr|\,u^{d-2}\,du .\]
\emph{Claim:} $\displaystyle L(d)=-\frac{\pi}{d-1}\cot\frac{\pi}{d}$.

Integrate by parts against $dv=u^{d-2}du$, $v=u^{d-1}/(d-1)$, using
$\frac{d}{du}\log|1-u^{-d}|=\frac{d}{u(u^d-1)}$. The boundary terms at $0$ and
$\infty$ vanish: as $u\to0$, $\log|1-u^{-d}|\sim-d\log u$ against $u^{d-1}\to0$; as
$u\to\infty$, $\log|1-u^{-d}|\sim-u^{-d}$ against $u^{d-1}$, giving $O(1/u)$. At
$u=1$ the two boundary terms do \emph{not} vanish individually --- excising
$(1-\delta,1+\delta)$ leaves $\frac{1}{d-1}\log(d\delta)$ at $1-\delta$ and
$-\frac{1}{d-1}\log(d\delta)$ at $1+\delta$, since
$1-(1\mp\delta)^{-d}=\mp d\delta+O(\delta^2)$ --- but they are equal and opposite, so
they cancel in the limit. Hence, as a principal value,
\[L(d)=-\frac{d}{d-1}\ \mathrm{PV}\!\!\int_0^\infty\frac{u^{d-2}}{u^{d}-1}\,du .\]
Substitute $t=u^d$ (monotone and smooth at $u=1$ with derivative $d$, so the
symmetric excision is preserved to first order and the principal value carries over):
$u^{d-2}du=\frac1d t^{-1/d}dt$, giving
\[\mathrm{PV}\!\!\int_0^\infty\frac{u^{d-2}}{u^{d}-1}du
 =\frac1d\,\mathrm{PV}\!\!\int_0^\infty\frac{t^{-1/d}}{t-1}\,dt
 =-\frac1d\,\mathrm{PV}\!\!\int_0^\infty\frac{t^{a-1}}{1-t}\,dt,\qquad a=1-\tfrac1d .\]
By the classical evaluation $\mathrm{PV}\int_0^\infty\frac{t^{a-1}}{1-t}dt=\pi\cot(\pi a)$
for $0<a<1$, and $\cot\bigl(\pi-\frac\pi d\bigr)=-\cot\frac\pi d$, this is
$\frac1d\pi\cot\frac\pi d$. Therefore
$L(d)=-\frac{d}{d-1}\cdot\frac{\pi}{d}\cot\frac{\pi}{d}
     =-\frac{\pi}{d-1}\cot\frac{\pi}{d}$, as claimed.

\emph{Consequence.} $\cot\frac\pi d>0$ exactly when $\frac\pi d<\frac\pi2$, i.e.\ when
$d>2$. So
\[\boxed{\,L(d)<0\ \text{ for } d\ge3,\qquad L(2)=0\,.}\]
The hypothesis $d\ge3$ is not a convenience here: at $d=2$ the quantity that has to be
negative vanishes identically.

\medskip
\noindent\textbf{Step 2: $J(p)<0$ for small $p$.} Pointwise,
$\frac{|\Phi_0|^p-1}{p}\to\log|\Phi_0|$ as $p\to0$. For $0<p\le p_1<\frac{d-1}{d}$ the
elementary bound $\bigl|\frac{a^p-1}{p}\bigr|\le|\log a|\max(1,a^p)$ gives the
dominating function
$\bigl|\log|\Phi_0|\bigr|\max\bigl(1,|\Phi_0|^{p_1}\bigr)u^{d-2}$, which is integrable:
near $u=0$ it is $O\bigl(u^{d-2-dp_1}\log\frac1u\bigr)$, near $u=1$ it is
$O\bigl(|\log|u-1||\bigr)$, and near $\infty$ it is $O(u^{-2})$. Dominated convergence
gives
\[\frac{J(p)}{p}\longrightarrow \omega_{d-2}\,L(d)<0\qquad(p\to0),\]
so there is $p_0\in(0,\frac{d-1}{d})$ with
$J(p)\le\tfrac12 p\,\omega_{d-2}L(d)<0$ for all $p\in(0,p_0)$.

\medskip
\noindent\textbf{Step 3: $I(\varepsilon,p)\to J(p)$ as $\varepsilon\to0$, $p$ fixed.}
Split at $|x|=\varepsilon$. On $\{|x|\le\varepsilon\}$, \eqref{eq:Phieps} gives
$|\Phi_\varepsilon|\le1+(d-1)\varepsilon^{-d}$, so
$\bigl|\int_{|x|\le\varepsilon}(|\Phi_\varepsilon|^p-1)\bigr|
 \lesssim\varepsilon^{-dp}\varepsilon^{d-1}=\varepsilon^{d-1-dp}\to0$, and likewise
$\bigl|\int_{|x|\le\varepsilon}(|\Phi_0|^p-1)\bigr|
 \lesssim\int_0^\varepsilon u^{-dp}u^{d-2}du
 =\frac{\varepsilon^{d-1-dp}}{d-1-dp}\to0$, both because $p<\frac{d-1}{d}$.
On $\{|x|>\varepsilon\}$ the integrands converge pointwise off the two zero sets, which
are null. For domination, note $\bigl||\Phi|^p-1\bigr|\le1+|\Phi|^p$; for
$\varepsilon<|x|\le2$ we have $|\Phi_\varepsilon|\lesssim|x|^{-d}$, giving the
integrable majorant $\bigl(1+C|x|^{-dp}\bigr)\mathbf 1_{|x|\le2}$, and for $|x|>2$,
$|\Phi_\varepsilon-1|\lesssim|x|^{-d}\le\frac12$, so Background \ref{bg:mvt} gives
$\bigl||\Phi_\varepsilon|^p-1\bigr|\lesssim|x|^{-d}$, integrable on $\Rdm$. Dominated
convergence applies on each piece.

\medskip
\noindent\textbf{Conclusion.} Fix $p\in(0,p_0)$ and set
$2M:=\tfrac14 p\,\omega_{d-2}|L(d)|>0$. By Step 2,
$J(p)\le-\tfrac12p\,\omega_{d-2}|L(d)|=-4M$; by Step 3 there is $\varepsilon_0(p,d)$
with $|I(\varepsilon,p)-J(p)|<2M$ for $\varepsilon<\varepsilon_0$. Hence
$I(\varepsilon,p)<-2M$ for all such $\varepsilon$, with $M$ independent of
$\varepsilon$ --- which is exactly what Lemma 1 requires of \eqref{eq:AK}, and exactly
the independence the phrase ``$\eta>0$ independent of $\varepsilon$'' asks for.
For $d=2$, $L(2)=0$ and the same computation gives $J(p)>0$ for small $p$; the
construction cannot work there, as it must not. \hfill$\blacksquare$

\medskip
\noindent\emph{Numerical corroboration.} All three steps were checked independently:
$L(d)$ against $-\frac{\pi}{d-1}\cot\frac\pi d$ to 19 significant digits at $d=3$ and
14 at $d=4$; and $I(\varepsilon,p)$ computed directly from \eqref{eq:Phieps}, which at
$d=3$, $p=0.05$ gives $-0.3005,\,-0.2569,\,-0.2559$ for
$\varepsilon=10^{-1},10^{-2},10^{-3}$ against $J(0.05)=-0.2558$; while at $d=2$ the
same computation returns $+0.0266$.
\end{filled}
...
\begin{caution}
\textbf{Repaired immediately below; this is not an open gap.}\par\smallskip
...
\begin{equation}\label{eq:L2}
 \Bigl(\int_Q\Bigl|I(x)+I(a_Q)\,\nd F_\varepsilon
   \bigl(N\ell(Q)^{-1}(x-a_Q)\bigr)\Bigr|^p dx
 \Bigr)^{1/p}<e^{-2\beta}\,|I(a_Q)|\,|Q|^{1/p}.
\end{equation}
\end{lemB}

\begin{filled}
\textbf{The factor $I(a_Q)$ is ours, and the lemma is false without it.}
\Ledger{C9} This is the one substantive correction in the digestion; everything
else on the list in \S0 is a typographical or sign slip.

The original prints \eqref{eq:L2} with $\nd F_\varepsilon(\cdots)$ \emph{not}
multiplied by $I(a_Q)$. Three observations force the factor.

\emph{(i) The printed statement is not homogeneous.} Replace $I$ by $cI$ for a
constant $c>0$. The hypothesis \eqref{eq:L2hyp} is unchanged, since $c$ cancels
between $|I(a_Q)|^{-1}$ and $\sup|I-I(a_Q)|$. The right-hand side of the printed
\eqref{eq:L2} scales by $c$. The left-hand side does not: it becomes
$(\int_Q|cI+\nd F_\varepsilon|^p)^{1/p}$, which is not $c$ times anything, because
$\nd F_\varepsilon$ stays put. So the printed statement asserts strictly more for
large $c$ than for $c=1$, from a hypothesis that does not distinguish them. Taking
$c\to\infty$ with $I$ constant makes the left side $\sim c|Q|^{1/p}$ and the right
side $c\,e^{-2\beta}|Q|^{1/p}$: false, since $e^{-2\beta}<1$. \emph{The printed
statement is therefore not merely unproved but untrue as it stands.}

\emph{(ii) The proof proves the normalised case.} The proof reduces to $Q=Q(N)$ and
then to ``if $N^{d-1}\|I-1\|_\infty$ is sufficiently small then
$\int_{Q(N)}(|I+\nd F_\varepsilon|^p-1)\le-\eta$''. Writing $\|I-1\|_\infty$ and
comparing against $1$ presumes $I(a_Q)=1$: it is exactly the normalised case, and
nothing else is established.

\emph{(iii) The application needs the factor.} This is a forward reference --- the
construction is \S4, the application \S6.3 --- but it is the reason the factor is not
optional, so it belongs with the statement. In \S6.3 Lemma 2 is applied to
$\nd u_{n+1}$ on a cube, which by \eqref{eq:decomp} is
$I(x)+\nd u_n(a_Q)\,\nd F_{\varepsilon_{n+1}}(N\delta_{n+1}^{-1}(x-a_Q))$ with
$I=\nd u_n(a_Q)+\mathrm{Br}$, so $I(a_Q)\approx\nd u_n(a_Q)$. The correction term
carries the coefficient $\nd u_n(a_Q)$ because that is how $u_{n+1}$ is defined in
\eqref{eq:un1} of \S4.2. So the quantity actually estimated is the one in the \emph{corrected}
\eqref{eq:L2}, not the printed one.

With the factor restored, both sides of \eqref{eq:L2} are homogeneous of degree $1$
in $I$, the hypothesis is scale-invariant, and \S3.1 reduces the general case to the
normalised one that the proof establishes. Statement, proof and application then
agree.

\smallskip
\emph{A second repair, and a milder diagnosis.} Andrade's thesis (\S9(6)) states this
lemma exactly as the original does, factor and all --- so the defect is in the
statement, not in our reading of it. But its \emph{application} takes a different
route: rather than restating the lemma, it factors the amplitude out by hand before
applying it, writing
\[\nd u_{n+1}(x)=\nd u_n(a_Q)\Bigl[1+\nd F_{\varepsilon_{n+1}}
   \bigl(N\delta_{n+1}^{-1}(x-a_Q)\bigr)\Bigr]+J(x),\]
and then invoking the lemma only in the case $I\equiv1$, with everything else absorbed
into the remainder $J$. That works, and it suggests a milder diagnosis: the printed
lemma is not so much wrong as \emph{over-stated}. Its proof gives the normalised case;
the normalised case is all anyone ever needs; and the general-$I$ form, which is what
fails, is never actually used. We keep the restated version, because a lemma one
cannot apply as written is worth flagging --- but either repair closes the gap.
\end{filled}
...
\[\int_Q\bigl|I(x)+I(a_Q)\,\nd F_\varepsilon(\Phi(x))\bigr|^pdx
 =\frac{|Q|}{|Q(N)|}\int_{Q(N)}
   \bigl|\tilde I(y)+\tilde I(0)\,\nd F_\varepsilon(y)\bigr|^pdy ,\]
because $dx=(\ell(Q)/N)^{d-1}dy=\frac{|Q|}{|Q(N)|}dy$ and $\tilde I(0)=I(a_Q)$.
Since $|Q|^{1/p}$ appears on the right of \eqref{eq:L2}, the inequality is
equivalent to
$\frac1{|Q(N)|}\int_{Q(N)}|\tilde I+\tilde I(0)\nd F_\varepsilon|^p
 <e^{-2\beta p}|\tilde I(0)|^p$,
...
\emph{Normalisation.} Here the corrected factor $I(a_Q)$ earns its keep. Set
$c=\tilde I(0)$, which we may assume $\ne0$ (otherwise \eqref{eq:L2hyp} is
undefined), and put $J=\tilde I/c$, so $J(0)=1$. Then
\[\tilde I(y)+\tilde I(0)\,\nd F_\varepsilon(y)
   =c\bigl(J(y)+\nd F_\varepsilon(y)\bigr),\]
so the left-hand side of \eqref{eq:L2} is exactly $|c|$ times the left-hand side for
$J$, and the right-hand side is exactly $|c|$ times the right-hand side for $J$.
Both sides are homogeneous of degree $1$ in $I$ --- \emph{which is true only with
the factor} --- and the hypothesis \eqref{eq:L2hyp} is unchanged, being
$N^{d-1}|J(0)|^{-1}\sup|J-J(0)|=N^{d-1}\|J-1\|_\infty$. So it suffices to treat
$J$, i.e. to assume $I(a_Q)=1$; and then \eqref{eq:L2hyp} reads
$N^{d-1}\|I-1\|_\infty$ and \eqref{eq:L2} reads \eqref{eq:L2red} with
$e^{-2\beta p}=1-\eta/N^{d-1}$; see \S3.4. Note that $F_\varepsilon$ is never
rescaled: it is the fixed function Lemma 1 produced, exactly as it must be.
\end{expansion}
...
\begin{expansion}
\textbf{This is the corrected Lemma 2, applied.} \Ledger{D17, C9} Put
\[I(x):=\nd u_n(a_Q)+\mathrm{Br}(x),\]
so that \eqref{eq:decomp} reads
\[\nd u_{n+1}(x)=I(x)+\nd u_n(a_Q)\,
   \nd F_{\varepsilon_{n+1}}\bigl(N\delta_{n+1}^{-1}(x-a_Q)\bigr).\]
The coefficient of $\nd F_{\varepsilon_{n+1}}$ is $\nd u_n(a_Q)$, and
$I(a_Q)=\nd u_n(a_Q)+\mathrm{Br}(a_Q)$ where
$|\mathrm{Br}(a_Q)|<CN^{-d}|\nd u_n(a_Q)|$, so
$I(a_Q)=(1+O(N^{-d}))\,\nd u_n(a_Q)$. Thus $\nd u_{n+1}$ is, up to that
$1+O(N^{-d})$ factor, exactly the integrand of the corrected \eqref{eq:L2} ---
$I(x)+I(a_Q)\nd F_\varepsilon(\cdots)$ --- and \emph{not} of the printed one. The
$O(N^{-d})$ discrepancy is absorbed into the constant $C$ below, $N$ being fixed.
This is the step that would fail with the statement as printed. Now
...
$\sum\varepsilon^{(d-1)/2}$. The exponent must be $(d-1)/2$, as written above. Andrade's thesis (\S9(6)) states condition (2) with the exponent $(d-1)/2$, independently of us --- so this correction is not a matter of taste.
...
\begin{expansion}
\Ledger{D32} First, why $V_n\setminus V_{n+1}$ is a union of stage-$(n{+}1)$
cubes at all: $\delta_n/\delta_{n+1}\in\mathbb Z$, so the grid $\mathcal H_{n+1}$
refines $\mathcal H_n$ exactly and every $Q'\in\mathcal Q_n$ is partitioned by cubes
of $\mathcal H_{n+1}$. Such a $Q\subset Q'$ satisfies (a) automatically, so it lies
in $\mathcal Q_{n+1}$ if and only if it satisfies (b); hence
$V_n\setminus V_{n+1}$ is precisely the union of those $Q$ failing (b), which is the
set the previous display applies to. (This is the only place the divisibility
$\delta_n/\delta_{n+1}\in\mathbb Z$ is used, and without it the two grids would not
nest and $V_{n+1}\subset V_n$ could fail.) Summing over those $Q$ --- disjoint and
contained in $V_n$ --- gives the first inequality; the second is \eqref{eq:IH}, $\int_{V_n}|\nd u_n|^p\le A^pe^{-p\beta n}$,
...
\item \textbf{The references, now traced.} \Ledger{E4, E5}

  \emph{[2] is identified.} T. Wolff, \emph{Counterexamples with harmonic gradients
  in $\mathbb{R}^3$}, in: C. Fefferman et al. (eds.), \emph{Essays on Fourier
  Analysis in Honor of Elias M.\ Stein}, Princeton Math.\ Ser.\ \textbf{42},
  Princeton Univ.\ Press, 321--384 (1995); Zbl 0836.31004. Verified from the zbMATH
  record; not read here.

  \emph{[1] is} A.B.~Aleksandrov and P.~Kargaev, \emph{Hardy classes of harmonic
  functions in a half-space}, Algebra i Analiz \textbf{5}, No.~2, 1--73 (1993);
  translated as St.\ Petersb.\ Math.\ J.\ \textbf{5}, No.~2, 229--286 (1994);
  Zbl 0842.42012. We did not obtain a copy, so \eqref{eq:AK} is not verified
  \emph{against their text}; it is instead proved from scratch in \S2.2, in the
  special case Lemma 1 needs. What Aleksandrov and Kargaev prove is presumably more
  general and differently organised, and comparing the two is the natural next step
  for anyone with access.

\item \textbf{The problem the paper leaves open has since been resolved --- in a
  preprint.} Remark 2 asks whether a harmonic function $C^2$ or $C^\infty$ up to the
  boundary can have $\nabla f$ vanishing on a boundary set of positive measure. On
  3~August 2026, three weeks before this digestion was written, G.~Qin and W.~Zhan
  posted \emph{Smooth Failure of Boundary Unique Continuation for Harmonic
  Functions} (arXiv:2608.01683 [math.AP]), whose abstract states that for every
  $n\ge3$ there is a nonconstant harmonic function on the half space, smooth up to
  the boundary, whose entire boundary jet vanishes on a compact nowhere dense
  boundary set of positive measure --- settling the smooth case in the negative. We
  have read the abstract and the metadata, not the proof, and it has not been
  refereed. The Open problems page of the site records this, with that provenance
  attached.
...
\item \textbf{The six slips.} Listed in \S0. Five are typographical or sign
  errors. The sixth --- the missing factor $I(a_Q)$ in Lemma 2 (\S3) --- is
  substantive: as printed the lemma is false, though the proof and the application
  both correspond to the corrected statement, so nothing downstream breaks. Two
  further points required a real repair rather than a correction: the lower bound
  for $|1+\nd G_\varepsilon|$ near $|x|=1$ (\S2.6, \S3.3), and the missing final
  step \S7.5.
\item \textbf{Nothing remains conditional.} \eqref{eq:AK} is proved in \S2.2, so
  the theorem is proved here outright.

\item \textbf{A second exposition, and what it settles.} R.~C.~D.~de Andrade,
  \emph{On zeros of gradients of harmonic functions on the upper half space},
  master's thesis, IMPA, supervised by J.~Ramos, 2 June 2026, 87 pp. A full and
  careful account of this paper: chapter~1 gives the Calder\'on/EIT motivation used
  in \S0.5, chapter~2 treats $d=2$, and chapter~3 works through the $d\ge3$
  construction in detail. Three things it settles for us:
  \begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  \item It states condition (2) with the exponent $(d-1)/2$, confirming \Ledger{D23}
    independently.
  \item It states Lemma 2 exactly as the original does --- so \Ledger{C9} is a defect
    of the paper, not a misreading --- but applies it only after factoring the
    amplitude out by hand, which is a second and lighter repair (\S3).
  \item It quotes \eqref{eq:AK} without proof, exactly as the original does: ``this
    lemma is due to Aleksandrov and Kargaev, its proof is beyond our interest''. So
    the proof in \S2.2 is not a reconstruction of something already written down in
    the literature we have seen --- both accounts of this paper take the inequality
    on trust.
  \end{itemize}
  We read the thesis after the digestion was complete; it changed the two items above
  and supplied \S0.5, and confirmed the rest.

\item \textbf{The two-dimensional case, sharply.} \S0.5 and the
  \texttt{dimension-threshold} entry say only that $d=2$ fails ``because $f+i\tilde f$
  is analytic''. Chapter~2 of Andrade's thesis makes this precise, deriving it from
  the characterisation of moduli of $H^p$ functions: a nonnegative $h\in L^p(\mathbb R)$
  is $|f|$ for some $f\in H^p$ if and only if
  $\int\frac{\log h(t)}{1+t^2}\,dt>-\infty$. Vanishing on a set of positive measure
  makes that integral $-\infty$, which forces $f\equiv0$. We have not verified the
  chapter; the statement is recorded here as the sharp form of the obstruction.

\item \textbf{Where $d\ge3$ enters \eqref{eq:AK}.} This began as a heuristic and is
  now Step 1 of the proof in \S2.2, so it is recorded there rather than here. The
  short version: to leading order in $p$ the Alexandrov--Kargaev integral is
  $p\,\omega_{d-2}L(d)$ with $L(d)=-\frac{\pi}{d-1}\cot\frac{\pi}{d}$, negative exactly
  for $d\ge3$ and identically zero at $d=2$ --- the borderline, and precisely where the
  theorem must fail, since in the plane $f+i\tilde f$ is analytic and $f,\nabla f$
  vanishing on a set of positive boundary measure forces $f\equiv0$. The same
  computation with the outward normal gives a positive leading term in every
  dimension, which is the check that fixes the sign convention of \S2.1.
