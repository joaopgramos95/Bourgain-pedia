...
something the shape of the estimate settles. We do not reproduce it. \S9(5) records
a formal leading-order computation which is consistent with \eqref{eq:AK} for
$d\ge3$ and which degenerates exactly at $d=2$; it is not a proof, and this row
stays \textbf{OPEN-GAP}.
\end{caution}
...
\item \textbf{The references, now traced.} \Ledger{E4, E5}
  ...
  \emph{[1] is not.} The best candidate is A.B.~Aleksandrov and P.~Kargaev,
  \emph{Hardy classes of functions harmonic in the half-space}, St.\ Petersb.\ Math.\
  J.\ \textbf{5}, No.~2, 229--286 (1994), translated from Algebra Anal.\ \textbf{5},
  No.~2, 1--73 (1993); Zbl 0842.42012. The authors, the subject and the period all
  fit, but we have not read it and therefore cannot confirm that it contains
  \eqref{eq:AK}. Reading it is the single highest-value follow-up for this
  digestion: it is the only input we could not verify.
...
\item \textbf{Where $d\ge3$ seems to enter \eqref{eq:AK}: a formal computation.}
  \S7 exhibits one place the hypothesis $d\ge3$ is used (convergence of
  $\sum\varepsilon_n^{(d-1)/2}$). The following suggests that it is also built into
  the imported inequality, and it is the observation we would most like to see
  settled.

  Send $\varepsilon\to0$ first. By \eqref{eq:dnG}, $\nd G_\varepsilon\to-|x|^{-d}$
  pointwise on $\Rdm\setminus\{0\}$, so the integrand of \eqref{eq:AK} tends
  formally to $\bigl|1-|x|^{-d}\bigr|^p-1$. Expanding in $p$ about $p=0$ pointwise,
  $|1-|x|^{-d}|^p-1=p\log\bigl|1-|x|^{-d}\bigr|+O(p^2)$, so the leading behaviour of
  \eqref{eq:AK} is $p\,\omega_{d-2}\,L(d)$ with
  \[L(d)=\int_0^\infty\log\bigl|1-u^{-d}\bigr|\,u^{d-2}\,du .\]
  This converges for $d\ge2$: the logarithmic singularity at $u=1$ is integrable,
  the integrand is $\sim-u^{-2}$ at infinity, and $\sim-d\,u^{d-2}\log u$ at $0$.
  Numerically,
  \[L(d)=-\frac{\pi}{d-1}\cot\frac{\pi}{d}\]
  to $19$ significant digits at $d=3$, $14$ at $d=4$, and $9$ at $d=6$ (the loss is
  quadrature error at the singularity). We have \emph{not} proved this closed form.
  What matters is its sign:
  \[L(2)=0,\qquad L(d)<0\ \text{ for }d\ge3,\]
  since $\cot(\pi/d)>0$ exactly when $d>2$, and $\cot(\pi/2)=0$. At $d=3$,
  $L(3)=-\pi/(2\sqrt3)$; at $d=4$, $L(4)=-\pi/3$.

  So at leading order in $p$ the Alexandrov--Kargaev integral is negative precisely
  when $d\ge3$, and its leading term \emph{vanishes identically} at $d=2$ --- the
  borderline, which is where the theorem itself must fail, since in two dimensions
  $f+i\tilde f$ is analytic and $f,\nabla f$ vanishing on a set of positive boundary
  measure forces $f\equiv0$.

  \emph{This is not a proof of anything.} Two interchanges are unjustified (the
  $\varepsilon\to0$ limit through the integral, and the expansion in $p$ under it),
  the closed form is numerical, and \eqref{eq:AK} requires a quantitative
  $-2M<0$ at a fixed small $p$ uniformly in small $\varepsilon$, which a leading-order
  sign does not supply. It is offered as the reason one should expect \eqref{eq:AK}
  to hold for $d\ge3$ and to fail at $d=2$, and as a check that our sign convention
  (\S2.1) is the right one --- with the outward normal the same computation gives a
  positive leading term in every dimension.
\end{enumerate}
